...
export default function DetailsMembers({channel}) {
  ...
  return (
    <div className='channel-detail-members'>
      <div className='channel-detail-member-div'>
      ...
      {members?.map(member => {
        return (
          <div className='channel-detail-member-div'>
            ...
            {member.is_active && <i className="fa-solid fa-circle" style={{ 'color': '#007a5a', 'width': '15px', 'height': 'auto', 'fontSize': '10px' }}></i>}
            {!member.is_active && <i className="fa-regular fa-circle" style={{ 'width': '15px', 'height': 'auto', 'fontSize': '10px' }}></i>}
          </div>
        )
      })}
    </div>
